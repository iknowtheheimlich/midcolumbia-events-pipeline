...
def test_load_config_and_explicit_override_precedence(tmp_path: Path) -> None:
    ...
    config = load_review_operations_config(path).with_overrides(stale_after=7, capacity_lookback=3)
...
def test_finalizer_persists_effective_configuration(tmp_path: Path) -> None:
    ...
    persisted = json.loads(Path(result["review_operations_config"]).read_text(encoding="utf-8"))
