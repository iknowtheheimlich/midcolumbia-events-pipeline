...
def _adapter(
    ...
    *,
    ...
    return AdapterInfo(
        ...
        include_in_attribution=include,
    )


def test_single_source_attribution():
    assert build_source_attribution([_adapter("A", "Source A")]) == (
        "This is not an all inclusive list. Events were extracted from Source A."
    )
...
def test_two_source_attribution_uses_and():
    assert build_source_attribution([
        _adapter("A", "Source A"),
        _adapter("B", "Source B"),
    ]) == "This is not an all inclusive list. Events were extracted from Source A and Source B."
...
def test_three_source_attribution_uses_oxford_comma():
    assert build_source_attribution([
        ...
        _adapter("B", "Source B"),
        _adapter("C", "Source C"),
    ]) == "This is not an all inclusive list. Events were extracted from Source A, Source B, and Source C."


def test_internal_source_is_excluded():
    assert build_source_attribution([
        _adapter("Public", "Public Source"),
        _adapter("Internal", None, include=False),
    ]) == "This is not an all inclusive list. Events were extracted from Public Source."


def test_duplicate_labels_are_collapsed_without_reordering():
    assert build_source_attribution([
        _adapter("A", "Shared Source"),
        _adapter("B", "Shared Source"),
        _adapter("C", "Other Source"),
    ]) == "This is not an all inclusive list. Events were extracted from Shared Source and Other Source."
